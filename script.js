...
function getPasswordOptions() {
 var length = parseInt(
      prompt('How many characters in pw?')
    );
...
if (length < 8) {
      alert('Password length must 5 characters');
      return;
    }
  
if (length > 128) {
      alert('Password length must 50 characters');
      return;
    }
...
var specialCharacters = ['@','%','+','\\','/',"'",'!','#','$','^','?',':',',',')','(','}','{',']','[','~','-','_','.'
  ...
  function getPasswordOptions() {
    ...
    if (length > 128) {
      alert('Password length must less than 122 characters');
      return;
    }
